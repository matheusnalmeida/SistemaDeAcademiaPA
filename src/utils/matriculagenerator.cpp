...
void MatriculaGenerator::validaCpf(QString cpf){
    ...
    for(int i = 0;i < cpf.length()-3;i++){
        if(cpf.at(i) != '.'){
            result_digito1 += arr_verificador_digito1[i] * cpf.at(i).digitValue();
        }
    }
    ...
    for(int i = 0;i < cpf.length()-1;i++){
        if(i == cpf.length()-2){
            result_digito2 += arr_verificador_digito2[i] * decimo;
            break;
        }
        if(cpf.at(i) != '.' && cpf.at(i) != "-"){
            result_digito2 += arr_verificador_digito2[i] * cpf.at(i).digitValue();
        }
    }
    ...
    if(decimo != cpf.at(12) || decimoPrimeiro != cpf.at(13)){
